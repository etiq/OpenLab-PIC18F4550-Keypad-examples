 ...
 * Description          : Example_10.1 Keypad_2
 *                      : Created on November 4, 2013, 4:30 PM
 *                      : Example code for detecting a keypad entry and displaying the input through UART (PULL UP mode)
 */
...
/*
  CONTROL PORT
  ------------
  PORTD/PORTB (define PORTB_CTRL / PORT D_CTRL in keyFn.c file for selecting control port)
...
void main(void)
    ...
    ConfigUSART(config_1, config_2);                                                  /* Invoking UART configuration function             */
     while(1)
       {
       GetKey();                                                    /* Key press detection function invoke              */
